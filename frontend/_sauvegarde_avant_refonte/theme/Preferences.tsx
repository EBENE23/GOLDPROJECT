...
import { Bell, BellRing, LogOut, Moon, Palette, Sun } from "lucide-react";
...
export default function Preferences() {
  ...
  const modifier = (changement: Partial<UserPreferences>, message?: string) => {
    ...
    if (changement.theme) applyAppTheme(changement.theme);
  ...
  const themes: { valeur: AppTheme; libelle: string; icone: typeof Sun; apercu: string }[] = [
    { valeur: "clair", libelle: "Clair", icone: Sun, apercu: "bg-white border-slate-200" },
    { valeur: "sombre", libelle: "Sombre", icone: Moon, apercu: "bg-slate-800 border-slate-700" },
  ];
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {themes.map(({ valeur, libelle, icone: Icone, apercu }) => {
            ...
            return (
              ...
              >
                <span className={`mb-3 flex h-14 items-end gap-1.5 rounded-xl border p-2 ${apercu}`}>
